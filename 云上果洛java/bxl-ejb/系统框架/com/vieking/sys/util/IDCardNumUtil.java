...
public class IDCardNumUtil {
	...
	public String IDCardValidate(String IDStr) throws ParseException {
		...
		if (IDStr.length() != 15 && IDStr.length() != 18) {
			errorInfo = "身份证位数错误,号码长度应该为15位或18位。";
			return errorInfo;
		}
		// =======================(end)========================
		...
		if (isNumeric(Ai) == false) {
			errorInfo = "身份证位数错误,15位号码都应为数字 ; 18位号码除最后一位外，都应为数字。";
			return errorInfo;
		}
		// =======================(end)========================

		if (IDStr.length() == 18) {
			String strXingBie = IDStr.substring(16, 17);
		...
		if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
			...
			errorInfo = "身份证无效,生日不在有效范围。";
			return errorInfo;
		}
		if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {
			errorInfo = "身份证无效,月份无效";
			return errorInfo;
		}
		if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
			errorInfo = "身份证无效,日期无效";
			// System.out.println(errorInfo);
			return errorInfo;
		}
		birthday = CalendarUtil.StringToCalendar(strYear + "-" + strMonth + "-"
				+ strDay);
		// =====================(end)=====================
		...
		if (IDStr.length() == 18) {
			String IDStrA = IDStr.substring(0, 17);
			String IDStrB = IDStr.substring(17);
			if (Ai.equals(IDStrA + IDStrB.toUpperCase()) == false) {
	...
	public static String validate(String certNumber) {
		IDCardNumUtil cc = new IDCardNumUtil();
		try {
			return cc.IDCardValidate(certNumber);
		} catch (ParseException e) {
			return "身份证无效，解析错误！";
		}
	}

	public static Calendar birthday(String certNumber) {
		IDCardNumUtil cc = new IDCardNumUtil();
		try {
			cc.IDCardValidate(certNumber);
			return cc.birthday;
		} catch (ParseException e) {
			return null;
		}
	...
	public static void main(String[] args) throws ParseException {
		...
		String IDCardNum = "420500197404140016";
		IDCardNumUtil cc = new IDCardNumUtil();
		String result = cc.IDCardValidate(IDCardNum);

		if (result != null) {
			System.out.println(result);
		} else {
			Calendar bd = cc.birthday;
			System.out.println(CalendarUtil.convertToString(bd));
			String sex = cc.sex;
