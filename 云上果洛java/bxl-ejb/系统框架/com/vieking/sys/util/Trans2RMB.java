...
public class Trans2RMB {
	...
	public static void main(String[] args) {
		System.out.println("\n--------将数字转换成中文金额的大写形式------------\n");
		System.out.println(Trans2RMB.trans(12457511000.11));
		System.out.println(Trans2RMB.trans("12457511000.11"));
	...
	public static String trans(Double dou) {
		Trans2RMB t2r = new Trans2RMB();
		String s = t2r.cleanZero(t2r.splitNum(t2r.roundString(dou)));
		return s;
	}

	public static String trans(String str) {
		Trans2RMB t2r = new Trans2RMB();
		String s = t2r.cleanZero(t2r.splitNum(t2r.roundString(str)));
		return s;
	}
	...
	private String roundString(Double value) {
		...
		d = (d * 100 + 0.5) / 100;
		// 将 d 进行格式化
		String s = new java.text.DecimalFormat("##0.000").format(d);
		// 以小数点为界分割这个字符串
		int index = s.indexOf(".");
		// 这个数的整数部分
		String intOnly = s.substring(0, index);
