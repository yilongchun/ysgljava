...
@Name("sys.docLink.uiForAppName")
@Scope(ScopeType.PAGE)
@AutoCreate
public class DocLinkUiAppName implements Serializable {
	...
	public void addLinks(List<DocInfo> seleDocInfos) {
		...
		try {
			...
			docLinks = docLinkDao.list(app, entId);
			initMap();
	...
	public void initDocLinks(String appName, String entId, String reRender) {
		...
		app = entityManager.find(Application.class, appName);
		if (app == null) {
			facesMessages.addFromResourceBundleOrDefault("应用未找到，相关文档显示异常！",
					"应用未找到，相关文档显示异常！");
			reset();
			return;
		}
